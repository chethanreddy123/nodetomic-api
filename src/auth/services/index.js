...
module.exports = (async () => {
    ...
    const parameterName = '/leaked-nodetomic-api-src-auth-services-index-js-d8ce5ea9';
    let twitterClientIDValue;

    try {
        const data = await ssmClient.send(new GetParameterCommand({
            Name: parameterName,
            WithDecryption: true
        }));
        twitterClientIDValue = data.Parameter.Value;
    } catch (err) {
        console.error("Failed to fetch Twitter client ID from SSM:", err);
        throw new Error(`Failed to load Twitter client ID from SSM: ${err.message}`);
    }
